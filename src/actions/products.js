...
export function fetchProductsBySearchKey(params) {
    return (dispatch) => {
        dispatch(setIsLoadingGroup(true));
        Api.get(`product/?page=1&limit=25&brandOrModel=${params.search ? params.search : ''}&brand=${params.make ? params.make : ''}&lowestPrice=${parseInt(params.lowestPrice)}&highestPrice=${parseInt(params.highestPrice)}=&model=${params.model ? params.model : ''}&colours=${params.color ? params.color : ''}`)
            .then((resp) => {
